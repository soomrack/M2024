...
struct Matrix matrix_make_ident(size_t rows, size_t cols) // Cоздание и возвращение единичной матрицы
{
...
void matrix_print(const struct Matrix A)  // Вывод матрицы на экран 
{
    if (A.data != NULL || A.cols != 0 || A.rows != 0) {
...
void matrix_add(const struct Matrix A, const struct Matrix B)  // Сложение матриц с перезаписью
{
