...
// Наполнение матрицы
void matrix_filling(Matrix *p_mtrx){
    printf("Сколько строк в вашей матрице?\n");
    scanf("%d", p_mtrx->rows);
    printf("Сколько столбцов в вашей матрице?\n");
    scanf("%d", p_mtrx->cols);
    printf("Последовательно введите элементы матрицы.\n");
    for (int i = 0; i < p_mtrx->rows; ++i) {
        for (int j = 0; j < p_mtrx->cols; ++j) {
            scanf("%d", p_mtrx->data[i][j]);
        }
        printf("\n");
    }
}
