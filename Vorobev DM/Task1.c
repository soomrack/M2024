...
struct Person {
    ...
    money food_spending;
    money rent;
    money loan_payment;  // вычисл€етс€ в функции compute_loan_payment
...
void calculation_loan_payment(struct Person*);  // вычисление ежемес€чной оплаты кредита

void init_BOB();
void BOBs_salary_increase(const int, const int);  // ежегодное повышение зарплаты
void BOBs_salary_getting(const int, const int);  // ежемес€чное начисление зарплаты
void BOB_buy_food(const int, const int);  // ежемес€чна€ покупка еды
void BOB_pay_rent(const int, const int);  // ежемес€чна€ оплата аренды
void BOB_pay_loan(const int, const int);  // ежемес€чна€ плата по кредиту
void BOBs_unexpectable_spends(const int, const int);  // ежемес€чный учет дополнительных расходов
void BOB_apartment_renovation(const int, const int);  // ежегодный ремонт квартиры
void BOBs_percents_by_deposit(const int, const int);  //  ежемес€чна€ выплата процентов по вкладу
void BOB_has_house();
void BOB_print_results();  // ¬ывод результатов Ѕоба

void init_ALICE();
void ALICEs_salary_increase(const int, const int);  // ежегодное повышение зарплаты
void ALICEs_salary_getting(const int, const int);  // ежемес€чное начисление зарплаты
void ALICE_buy_food(const int, const int);  // ежемес€чна€ покупка еды
void ALICE_pay_rent(const int, const int);  // ежемес€чна€ оплата аренды
void ALICE_pay_loan(const int, const int);  // ежемес€чна€ плата по кредиту
void ALICEs_unexpectable_spends(const int, const int);  // ежемес€чный учет дополнительных расходов
void ALICE_apartment_renovation(const int, const int);  // ежегодный ремонт квартиры
void ALICEs_percents_by_deposit(const int, const int);  //  ежемес€чна€ выплата процентов по вкладу
void ALICE_has_house();
void ALICE_print_results();  // ¬ывод результатов јлисы

void inflate(const int, const int);  // ежемес€чный рост цен
void calculation();  // вычисление результатов
...
void calculation_loan_payment(struct Person* person)
{
    ...
    if (person->loan_payment > person->salary) {
        printf("ѕрограмма остановлена: ѕлата по кредиту больше заработной платы");
        exit(0);
    }
}
