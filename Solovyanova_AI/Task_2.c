...
Matrix matrix_exp(Matrix matrix, const size_t iteration)
{
    ...
    Matrix result = matrix_locate(matrix.rows, matrix.cols);
    Matrix B = matrix_locate(matrix.rows, matrix.cols);
    ...
    for (size_t row = 0; row < matrix.rows; ++row) {
        for (size_t col = 0; col < matrix.cols; ++col) {
            if (row == col) {
                B.data[row * matrix.cols + col] = 1.0;
            }
            else {
                B.data[row * matrix.cols + col] = 0.0;
            }
        }
    }
    ...
    for (size_t row = 0; row < matrix.rows; ++row) {
        for (size_t col = 0; col < matrix.cols; ++col) {
            result.data[row * matrix.cols + col] = B.data[row * matrix.cols + col];
        }
    }

    for (size_t k = 1; k <= iteration; ++k) {
        B = matrix_mul(B, matrix);
