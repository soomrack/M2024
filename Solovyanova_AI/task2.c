...
Matrix matrix_exp(Matrix matrix, const size_t iteration)
{
    ...
    Matrix result = matrix_locate(matrix.rows, matrix.cols);
     ...
     for (size_t row = 0; row < matrix.rows; ++row) {
        for (size_t col = 0; col < matrix.cols; ++col) {
            if (row == col) {
                result.data[row * matrix.cols + col] = 1.0;
            }
            else {
                result.data[row * matrix.cols + col] = 0.0;
            }
        }
    }

    for (size_t n = 1; n <= iteration; ++n) {
        for (size_t i = 0; i < matrix.rows; ++i) {
            for (size_t j = 0; j < matrix.cols; ++j) {
                double sum = 0.0;
                for (size_t k = 0; k < matrix.rows; ++k) {
                    sum += result.data[i * matrix.cols + k] * matrix.data[k * matrix.cols + j];
                }
                result.data[i * matrix.cols + j] += sum / (double)n;
            }
        }
    }
    return result;
