...
Matrix matrix_sum(const Matrix A, const Matrix B)
{
    if (A.rows != B.rows || A.cols != B.cols) {
        matrix_error_of_size("Add", "Matrixes must have equal sizes");
        return matrix_zero;
    }
...
{
    if (A.rows != B.rows && A.cols != B.cols) {
        matrix_error_of_size("Subtruction", "Matrixes must have equal sizes");
        return matrix_zero;
    }
...
{
    if (A.cols != B.rows) {
        matrix_error_of_size("Multiplication", "Matrixes must have certain sizes");
        return matrix_zero;
    }
    ...
    for (size_t rowA = 0; rowA < A.rows; ++rowA) {
        for (size_t colB = 0; colB < B.cols; ++colB) {
            ...
            for (size_t k = 0; k < A.cols; ++k) {
                sum += A.data[rowA * A.cols + k] * B.data[k * B.cols + colB];
            }
            result.data[rowA * B.cols + colB] = sum;
        }
    }
    return result;
}
...
MatrixItem matrix_det(const Matrix matrix)
{
    if (matrix.cols != matrix.rows) {
        matrix_error_of_size("Getting determinant", "Matrix must be square");
        return NAN;
    }
    ...
    if (n == 0) {
        matrix_error_of_size("Getting determinant", "Matrix must not be NULL");
        return NAN;
    }
...
{
    if (matrix.cols != matrix.rows) {
        matrix_error_of_size("Exp", "Matrix must be square");
        return matrix_zero;
    }
