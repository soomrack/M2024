...
{
    if (A.cols != B.rows) {
        matrix_error_of_size("Multiplication", "Matrixes should have certain sizes");
        return MATRIX_ZERO;
    }
...
MatrixItem matrix_det(const Matrix matrix)
{
    ...
    if (n == 0) {
        matrix_error_of_size("Getting determinant", "Matrix shouldn't be NULL");
        return NAN;
    }
...
{
    if (matrix.cols != matrix.rows) {
        matrix_error_of_size("Exp", "Matrix should be square");
        return MATRIX_ZERO;
    }
